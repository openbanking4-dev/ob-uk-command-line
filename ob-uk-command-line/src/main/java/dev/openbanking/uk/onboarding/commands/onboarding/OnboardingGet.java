...
public class OnboardingGet implements Callable<Integer> {
    ...
    public Integer call() throws URISyntaxException, KeyStoreException, CertificateException, NoSuchAlgorithmException, SslConfigurationFailure, IOException {
        init();
        log.debug("ob on-board get");
        OIDCWellKnown wellKnown = onboarding.getWellKnown();
        try {
            OIDCRegistrationResponse oidcRegistrationResponse = dynamicRegistrationService.read(
            ...
            log.error("The ASPSP rejected the read of the OIDC client: {}", e.getResponseBodyAsString(), e);
        }
        return 43;
    }
